...
def valid_workspace(s, pat=re.compile(r"aws-a-(cl|np|pr)-(\d{7})-.*")):
    matcher = pat.match(s)
    if not matcher:
...
def add_authentication(workspace):
    ...
    if not authentication_config.has_option('roleMap_AWS-Prod',splunk_group_name):
        backup(authentication_file_name)
        authentication_config.set('roleMap_AWS-Prod',splunk_group_name, ad_group_name)
...
def remove_authentication(workspace):
    ...
    if authentication_config.has_option('roleMap_AWS-Prod',splunk_group_name):
        backup(authentication_file_name)
        authentication_config.remove_option('roleMap_AWS-Prod',splunk_group_name)
        write_file(authentication_config, authentication_file_name)
        updated = True
...
def apply_config(splunk_password):
    splunk_command = '/opt/splunk/bin/splunk apply shcluster-bundle --answer-yes -auth admin:{} -target https://splunk-search-head:8089 -preserve-lookups true -push-default-apps true'.format(splunk_password)
